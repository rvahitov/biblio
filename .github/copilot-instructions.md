...
Для автоматизированного Copilot Code Review используйте модель `gpt-5-mini`.
Укажите эту модель в настройках инструмента ревью или явно в подсказках (prompt), например: `model: gpt-5-mini`.
Модель `gpt-5-mini` даёт компактные и точные рекомендации по изменению кода и форматированию, поэтому она должна применяться для всех автоматических обзоров кода перед созданием Pull Request.
